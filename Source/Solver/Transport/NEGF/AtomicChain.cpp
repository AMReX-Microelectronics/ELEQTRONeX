...
void c_AtomicChain::Read_MaterialSpecificNanostructureProperties()
{
    ...
    amrex::Print() << "##### Reading ParmParse AtomicChain_Default\n";
    for (int i = 0; i < 2; ++i)
    {
        if (i == 1)
        {
...
void c_AtomicChain::Print_MaterialSpecificReadData() { Print_AtomicChainParameters(); }
...
s_Position3D c_AtomicChain::get_AtomPosition(int field_id)
{
    ...
    int Y_center_offset= -(num_unitcells/2);

    if(num_unitcells%2==1) Y_center_offset -= spacing/2.;
...
void c_AtomicChain::Construct_Hamiltonian()
{
    ...
    if((vec_cumu_blkCol_size[my_rank] <= 0 && 0 < vec_cumu_blkCol_size[my_rank+1])) {
